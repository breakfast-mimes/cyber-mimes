export const weapons = {
  FISTS: {name: "fists", type: "unarmed", dmg: 0, stat: "str", equip: "rightHand", e: true},
  BROAD_SWORD: {name: "Board Sword", type: "melee", dmg: 3, stat: "str", equip: "rightHand", e: false},
  LAZER_GUN: {name: "Lazer Gun", type: "ranged", dmg: 3, stat: "dex", equip: "rightHand", e: false},
  ANTIMATTER_GUN: {name: "Antimatter Gun", type: "ranged", dmg: 10, stat: "dex"}
}

export const head = {
  LEATHER_CAP: {name: "Leather Cap", type: "armor", armor: 5, equip: "head"}
}

export const body = {
  BROKEN_POWER_ARMOR: {name: "Broken Power Armor", type: "armor", armor: 5, equip: "body"}
}